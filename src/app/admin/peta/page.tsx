...
const MapDisplay = dynamic(() => import('@/components/MapDisplay'), {
  ...
  loading: () => (
    <div className="w-full h-[650px] bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400 text-xs">
      Memuat Peta GIS Sebaran IKM 15 Kabupaten/Kota Sulawesi Utara...
    </div>
  ),
});
...
export default function AdminPetaPage() {
  ...
  const fetchMerchants = async () => {
    setLoading(true);
    try {
      let url = `/api/merchants?limit=300`;
    ...
    <AdminSidebarLayout user={userSession}>
      <main className="w-full px-3 sm:px-5 lg:px-6 py-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-100 text-sky-900 border border-sky-200 text-xs font-bold uppercase mb-2">
              ...
              GIS Geospasial Provinsi Sulawesi Utara
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
              Pemetaan Sentra IKM Sulawesi Utara
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Pemetaan titik koordinat lokasi workshop, pabrik, dan gerai IKM di 15 Kabupaten/Kota se-Sulawesi Utara.
            </p>
          </div>

          <div className="flex items-center gap-2 text-xs font-bold text-sky-900 bg-white px-4 py-2.5 rounded-xl border border-slate-200 shadow-xs">
            <Building2 className="w-4 h-4 text-sky-600" />
            <span>Terpetakan: <strong className="text-slate-900">{merchants.length}</strong> Titik IKM</span>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col lg:flex-row items-center gap-3">
          <form onSubmit={handleSearch} className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-20 py-2 text-xs rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-500"
            />
            <button
              type="submit"
              className="absolute right-1 top-1 bottom-1 px-3 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-xs font-bold transition-colors"
            >
              Cari
            </button>
          </form>
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <select
              ...
              className="text-xs py-2 px-3 rounded-xl border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-sky-500 w-full lg:w-56 font-medium"
            >
              <option value="ALL">Semua 15 Kab/Kota</option>
            ...
            <select
              ...
              className="text-xs py-2 px-3 rounded-xl border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-sky-500 w-full lg:w-52 font-medium"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* Side List */}
          <div className="lg:col-span-4 xl:col-span-3 bg-white rounded-2xl border border-slate-200 shadow-xs flex flex-col h-[600px]">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="font-bold text-xs text-slate-900 uppercase">Daftar Titik IKM</h3>
            ...
            <div className="overflow-y-auto flex-1 p-3 space-y-2">
              {loading ? (
                <div className="text-center py-12 text-xs text-slate-400">Memuat titik koordinat...</div>
              ) : merchants.length === 0 ? (
                    ...
                    >
                      ...
                      {isSelected && (
                        <div className="mt-2.5 pt-2 border-t border-sky-200/60 flex items-center justify-between gap-2">
                          ...
                          >
                            <Eye className="w-3 h-3" /> Surat STBP
                          </button>
                        </div>
                      )}
                    </div>
